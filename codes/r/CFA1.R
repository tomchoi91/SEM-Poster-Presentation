...
phi<- matrix(c(1, .5, .4, .5, 1, .3, .4, .3, 1), 3, byrow=TRUE)
phi
...
xi2  =~  NA*x4 + x6 + x7 + x8 + x9 + x10
xi3  =~  NA*x1 + x9 + x11 + x12 + x13 + x14 + x15
# factor covariances
xi1 ~~ 1*xi1
xi2 ~~ 1*xi2
xi3 ~~ 1*xi3
...
xi1  =~  NA*x1 + x2 + x3 + x4 + x5 
xi2  =~  0*x4 + x6 + x7 + x8 + x9 + x10
xi3  =~  0*x1 + x9 + x11 + x12 + x13 + x14 + x15
# factor covariances
xi1 ~~ 1*xi1
xi2 ~~ 1*xi2
xi3 ~~ 1*xi3
...
gen_data2 <- function(model, n, ...){
  ...
  colnames(y) <- c("x1",  "x2",  "x3",  "x4",  "x5",  "x6",  "x7",  "x8",  "x9",  "x10", "x11", "x12", "x13", "x14", "x15")
...
cfa1.fit <- gen_data4(cfa1.fit, w7, "CFA1", "big", "LV2", 1e3, "NA")
head(cfa1.fit)
...
cfa1.fit <- data.frame()
cfa1.fit <- gen_data5(cfa1.fit, h3, "CFA", "big", "lvl3", 1e6, "NA")
cfa1.fit
...
write.table(y, file = "cfa1_data.csv", row.names = FALSE, col.names = FALSE, sep = ",")
write.table(cfa1.fit, file = "cfa1_trial9.csv", row.names = FALSE, col.names = TRUE, sep = ",")
?write.table()
